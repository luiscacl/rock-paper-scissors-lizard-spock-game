...
const userIcons = document.querySelector('.section-one-user-choice .icons').children;
const computerIcons = document.querySelector('.section-two-computer-choice .icons').children;
const results = document.querySelector('.results');
const userScore = document.querySelector('.section-one-user-choice .score');
const computerScore = document.querySelector('.section-two-computer-choice .score');
...
console.log(choices.rock.defeats.indexOf('scissors'));
...
// Se recorren todos los iconos del usuario y se les añaden eventos
for (let i = 0; i < userIcons.length; i++) {
    const icon = userIcons[i];
    // iconId++;

    // Aquí es super importante definir la función que se va a ejecutar primero y después
...
function changeItemColor(choice){
    ...
    // ITEMS FROM COMPUTER
    let randomComputerIcon = computerIcons[Math.floor(Math.random() * computerIcons.length)];
...
// Esta función esta oculta
function decideWinner(){ 
    if(userChoice === computerChoice){
        results.innerHTML = "IT'S A TIE";
        stopConfetti(); 
        removeConfetti();
    } else if(choices[userChoice.toLowerCase()].defeats.indexOf(computerChoice.toLowerCase()) >= 0){
